...
static int c1;
static int c2=10;//权值右边
static double c3=4000;//调参权值左边
static int c4 = 4;//每增加一辆车，代价函数就是这么多
static int c5 = 3;//提前批多放多少辆车
static int c6= 10;//提前多少批
static int pici=1500;
...
//                        cout<<"1:  "<<pow((four/bandwidth)*c3,2)<<" 2  "<<bandwidth/(bandwidth2*speedlimit)<<endl;
                    ...
                    newdist=dist[outline][u]+(pretwo/(bandwidth))*c3+average+c2*(bandwidth/(bandwidth2*speedlimit));//weight公式
//                    cout<<c2*(pretwo/(bandwidth2*speedlimit))<<"  ";
                    //                    cout<<(pretwo/(bandwidth*speedlimit))<<endl;
                    if(maxtest1<(bandwidth/(bandwidth2*speedlimit)))
                        maxtest1=(bandwidth/(bandwidth2*speedlimit));
                }
                else
