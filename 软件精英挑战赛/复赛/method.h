 ...
 * {key:time ,value(key:roadID,value:weight)}*/
   ...
   void builTimeIndex(vector<Pre_set_car>);                      /*建立timeIndex*/
...
//    void initTraffic(vector<Car> cars, vector<Cross> crosses, vector<Road> roads);
    ...
    vector<Car>get_priority_or_unpriority_car(vector<Car>, bool priority);/*根据优先级将车辆分为两部份,便于后边操作*/
...
//    void change_car_start_time(pre_set_cars);
    ...
    int getTimeCostOf(Car* car,Road* road,Cross* cross);                         /*获得某个路口的平均权重,这个权重包括下歌路口的4条路的平均权重*/
