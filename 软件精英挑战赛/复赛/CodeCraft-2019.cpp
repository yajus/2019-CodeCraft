...
//static int c1;
//static int c2=10;
//static double c3=1500;//调参
//static int c4 = 4;//每增加一辆车，代价函数就是这么多
//static int c5 = 3;
//static int c6= 10;
//static int epoch = 10;/**调参调参调参**,批次**/
//struct car {
//    int from;
//    int to;
//    int speed;
...
//static hash_map<int,int> car_Index_ID;        //因为hash_map是非线性的,所以只能通过carIndex的下标来所以,存储的是carID
//static hash_map<int,int> timeIndex;       //时间片索引,下标为时间,值为时间片的车数
//static hash_map<int,int> road_Index_ID;        //道路索引
//static hash_map<int,int> road_ID_Index;
//static hash_map<int,int> cross_ID_Index;       //路口ID和文件所在行数的映射
//static hash_map<int,int> cross_Index_ID;
////用于hash_map的比较排序
...
////mark
//
//struct cmp_plantime  //自定义比较规则,出发时间
//{
//    bool operator()(const PAIR &P1, const PAIR &P2)  //注意是PAIR类型，需要.first和.second。这个和hash_map类似
//    {
//
...
////        cout<<time<<","<<chosenroad<<","<<frejudge<<","<<frequency[time][cross_ID_Index[chosenroad]][frejudge]<<endl;
////        cout<<"hello"<<endl;
//
...
//            car_Index_ID.insert(pair<int,int>(carline,carID));
...
//            road_Index_ID.insert(pair<int,int>(roadline,roadID));
//            road_ID_Index.insert(pair<int,int>(roadID,roadline));
...
//            cross_Index_ID.insert(pair<int,int>(crossline,crossID));
//            cross_ID_Index.insert(pair<int,int>(crossID,crossline));
...
//            int time = pre_answer_Info.time ;  //预制出发时间
//            int start_point = cardata[carID].from; //预制车的出发点
//            int speed; //车速度
//            for (iter; iter != result.end(); iter++) {
//                pre_answer_Info.path.push(stoi(*iter));//将预置的车的路线放入队列中
//                chosen_road = stoi(*iter); //当前路的ID
//                if (start_point == roaddata[chosen_road].from)
//                    frejudge = 1;
//                else if (start_point == roaddata[chosen_road].to)
//                    frejudge = 2;
//
//                frequency[time][cross_ID_Index[chosen_road]][frejudge] += 1;//添加了时间
//                start_point = start_point==roaddata[chosen_road].from?roaddata[chosen_road].to:roaddata[chosen_road].from;
//                speed = min(cardata[carID].speed,roaddata[chosen_road].speed);
//                time +=roaddata[chosen_road].length/speed;
//            }
...
////
//    const char *carargv = "/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/car.txt";
//    const char *roadargv = "/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/road.txt";
//    const char *crossargv = "/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/cross.txt";
//    const char *pre_answer_argv = "/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/presetAnswer.txt";
//
//    //读取数据
//    crossin(crossargv);
//    carin(carargv);
...
//    const char *answer_file = "/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/answer1.txt";
