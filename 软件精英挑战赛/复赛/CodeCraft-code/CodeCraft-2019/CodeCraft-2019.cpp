...
static int cardatacopy[110000][10]={0};//[路口数][属性]
static int cardataorigin[110000][10]={0};//[路口数][属性]
...
static int usedtime[110000]={0};
static int record[12000]={0};//使用次数
static int frejudge=-1;//From 1 To 2
static int c1;
static int c2=10;
static double c3=1800;//调参
static int c4 = 4;//每增加一辆车，代价函数就是这么多
static int c5 = 3;
static int c6= 10;
static int shuliang=100;
static int shuliang1=100;
static int pici=5000;
static int outline=0;
static int oo=1;

static int judgepreset[20000]={0};
static int frequencyweight[20000]={0};
static int frequencynumber[20000]={0};
...
//    cout<<tmp<<endl;
    ...
    for(int i=tot; i>1; --i)
    {
        ...
        usedtime[carnum]+=(roaddata[RoadIDToIndex[result]][2]/min(roaddata[RoadIDToIndex[result]][3],cardata[carnum][4]));//时间计算
//        cout<<usedtime[carnum]<<endl;
        pa++;
    }
    //        if(i != 1)
    //            cout << que[i] << " -> ";
...
//                int k= CarIDToIndex[cardata[i][1]];
                ...
                swap(cardataorigin[i],cardataorigin[j]);
//                int k;
...
//                k=CarIDToIndex[cardata[i][1]];
                CarIDToIndex[cardata[i][1]]=i;
                CarIDToIndex[cardata[j][1]]=j;
                long long k=usedtime[i];
                usedtime[i]=usedtime[j];
                usedtime[j]=k;
            }
        }
}

void carsort2()//根据usedtime排序
{
    for (int i=1;i<=carline;i++)
        for(int j=i+1;j<=carline;j++)
        {
            if(usedtime[i]<usedtime[j])
            {
                //                int k= CarIDToIndex[cardata[i][1]];
//                cout<<j<<"  "<<i<<endl;
                swap(cardata[i],cardata[j]);
...
void carsort3()//根据id排序
{
    for (int i=1;i<=carline;i++)
        for(int j=i+1;j<=carline;j++)
        {
            if(cardata[i][1]<cardata[j][1])
            {
                //                int k= CarIDToIndex[cardata[i][1]];
                swap(cardata[i],cardata[j]);
                swap(cardataorigin[i],cardataorigin[j]);
...
void presetsort()
{
    for(int i=1;i<=presetline;i++)
        for(int j=i+1;j<=presetline;j++)
        {
//            static int judgepreset[20000]={0};
//            static int frequencyweight[20000]={0};
//            static int frequencynumber[20000]={0};
            if(frequencyweight[i]/frequencynumber[i]<frequencyweight[j]/frequencynumber[j])
            {
                swap(presetdata[i],presetdata[j]);
                int k=judgepreset[i];
                judgepreset[i]=judgepreset[j];
                judgepreset[j]=k;
                k=frequencyweight[i];
                frequencyweight[i]=frequencyweight[j];
                frequencyweight[j]=k;
                k=frequencynumber[i];
                frequencynumber[i]=frequencynumber[j];
                frequencynumber[j]=k;
                PresetIDToIndex[presetdata[i][1]]=i;
                PresetIDToIndex[presetdata[j][1]]=j;
            }
        }
}
...
double calculaterms()
{
    ...
    for (int i=1;i<=carline;i++)
    {
//        cout<<cardatacopy[i][5]<<"  "<<cardata[i][5]<<endl;
        if(cardata[i][7]==1)
            continue;
            point++;
        sum+=(cardatacopy[i][5]-cardata[i][5])*(cardatacopy[i][5]-cardata[i][5]);
    }
    sum/=point;
    return sqrt(sum);
}
...
void ChangeStartTime1()//按照时间重新排序
{
    int i=1;
    int j=1;
    int point=1;
    while(point<=carline)
    {
        ...
        if(record[j]>shuliang1)
        {
            j++;
//            continue;
        }
        if(cardata[point][7]==1)
        {
            point++;
            continue;
        }
        cost+=((cardata[point][5]-max(j,cardataorigin[point][5]))*(cardata[point][5]-max(j,cardataorigin[point][5])));
        costnum++;
//        if(cardata[point][1]==10011)
//            cout<<max(j,cardataorigin[point][5])<<"   "<<cardataorigin[point][1];
        cardata[point][5]=max(j,cardataorigin[point][5]);
        record[cardata[point][5]]++;
        //        cout<<cardata[point][5]<<endl;
//        j++;
        point++;
...
//
    ...
    roadin(roadargv);
    carin(carargv);
     memcpy(cardataorigin,cardata,sizeof(cardata));
    int number=1;
    while(oo>0)
    {
        oo--;
...
//            carsort3();
//            memcpy(cardatacopy,cardata,sizeof(cardata));
            carsort2();
            costnum=0;
            cost=0;
            ChangeStartTime1();
//            carsort3();
//            jcs=calculaterms();
            jcs=sqrt(cost/costnum);
            cout<<"jcs"<<jcs<<endl;
            if(jcs<=70)
                break;
            carsort1();
        }
        memset(CarRoadID, 0, sizeof(CarRoadID));
        memset(dist, 0, sizeof(dist));
        memset(prev1, 0, sizeof(prev1));
        memset(usedtime, 0, sizeof(usedtime));
        cout<<"test";
        number++;
//    for(int i=1;i<=carline;i++)
//                cout<<CarIDToIndex[cardata[i][1]]<<endl;
    ...
    for(int i=1;i<=presetline;i++)
    {
        for(int j=3;;j++)
        {
            ...
            if(j==3)
            {
                ...
                if(crossdata[CorssIDToIndex[roaddata[RoadIDToIndex[presetdata[i][j]]][6]]][2]==presetdata[i][j+1]||crossdata[CorssIDToIndex[roaddata[RoadIDToIndex[presetdata[i][j]]][6]]][3]==presetdata[i][j+1]||crossdata[CorssIDToIndex[roaddata[RoadIDToIndex[presetdata[i][j]]][6]]][4]==presetdata[i][j+1]||crossdata[CorssIDToIndex[roaddata[RoadIDToIndex[presetdata[i][j]]][6]]][5]==presetdata[i][j+1])
                {
                    judge=1;
                }
                else
            ...
            frequencyweight[i]+=frequency[(presetdata[i][2]+3)/c4-1][RoadIDToIndex[presetdata[i][j]]][judge];//presetdata[i][2]+3/c4 批次
        }
    }
    presetsort();
    for(int i=1;i<=presetline*0.1;i++)
    {
        judgepreset[i]=1;
    }
...
// //////////////////////start
    ...
    for(outline=1;outline<=carline;outline++)
    {
        ...
        //        printf("\n");
        //    }
        if(cardata[outline][7]==1&&judgepreset[PresetIDToIndex[cardata[outline][1]]]==0)
            continue;
        int start=CorssIDToIndex[cardata[outline][2]];
        int end=CorssIDToIndex[cardata[outline][3]];
        Dijkstra(crossline, start,outline,c);
...
//    char* answer=argv[4];
    ...
    for(int i=1;i<=carline;i++)
    {
        if(cardata[i][1]==10002)
            cout<<cardata[i][7]<<"test"<<endl;
        if(cardata[i][7]==1&&judgepreset[PresetIDToIndex[cardata[outline][1]]]==0)
            continue;
